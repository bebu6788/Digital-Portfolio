...
public class FNCD implements Subject{ 
  ...
  public void runSimulation(int n) {
    Tracker track = new Tracker(); // instantiate a new tracker object
    addSubscriber(track); // add the tracker to the subject's subscriber list
    while (dayCount < n) {
      Day d = new Day();
      dayCount++;
      Logger log = new Logger(dayCount); // instantiate a new logger object
      addSubscriber(log); // add the logger to the subject's subscriber list

      System.out.println("*** FNCD Day " + Integer.toString(dayCount) + " ***");
      publishEvent(new Event("*** FNCD Day " + Integer.toString(dayCount) + " ***", 0, 0, dayCount)); // Publish the event to the subscribers
      d.simulateDay(this);
      removeSubscriber(log);
  ...
  public boolean addVehicle() {
    ...
    // Get 3 of each type of car and purchase it to FNCD inventory
  ...
  public void updateBudget() {
    if (budget <= 0) {
      ...
      publishEvent(new Event("Budget increased by $250000", 0, 0, dayCount));
    }
  }
}
