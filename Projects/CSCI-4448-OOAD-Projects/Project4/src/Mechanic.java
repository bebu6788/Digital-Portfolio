...
public class Mechanic extends Staff {
  ...
  // returns whether the mechanic earned a bonus for this repair
  public boolean repairVehicle(Vehicle v, FNCD dealer) {
    boolean ret = false;
    ...
    if (r <= 80) { // 80% chance of repairing any vehicle
      ...
      System.out.println("Mechanic " + getName() + " repaired " + v.getName() +
                         ...
                         + v.getBonusAmount() + ")\n");
      dealer.publishEvent(new Event("Mechanic " + getName() + " repaired " + v.getName() +
                          " and made it " + v.getCondition().toString() + " (earned $"
                          + v.getBonusAmount() + ")\n", 0, 0, dealer.getDays()));
    }
